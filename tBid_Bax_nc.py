...
_nominal_tBid_0 = 1 
_nominal_Bax_0 = 100
NUM_COMPARTMENTS = _scaling_factor * _nominal_Vesicles_0

RSF = 1. / _scaling_factor # Forward rate scaling factor
...
def initialize_model():
    ...
    for i in range(1, NUM_COMPARTMENTS+1):
        Compartment('c' + str(i), dimension=2, parent=solution)
    #}}}

    #{{{# INITIAL CONDITIONS
    Parameter('tBid_0', _scaling_factor * _nominal_tBid_0)
    Parameter('Bax_0', _scaling_factor * _nominal_Bax_0)

    Initial(tBid(bh3=None, loc='c') ** solution, tBid_0)
    Initial(Bax(bh3=None, a6=None, loc='c') ** solution, Bax_0)
    #}}}

    #{{{# OBSERVABLES
    Observable('ctBid', tBid(loc='c') ** solution)
    Observable('mtBid', tBid(loc='m'))
    Observable('cBax', Bax(loc='c') ** solution)
    Observable('mBax', Bax(loc='m'))

    # Activation
    #Observable('iBax', Bax(loc='i'))
    Observable('tBidBax', tBid(loc='m', bh3=1) % Bax(loc='m', a6=1))

    # pore formation
    for i, cpt in enumerate(model.compartments):
        if (not cpt.name == 'solution'):
            Observable('tBid_%s' % cpt.name, tBid() ** cpt)
            Observable('Bax_%s' % cpt.name, Bax() ** cpt)
            #Observable('iBax_%s' % cpt.name, Bax(loc='i') ** cpt)
            Observable('tBidBax_%s' % cpt.name,
                        tBid(loc='m', bh3=1) % Bax(loc='m', a6=1) ** cpt)
            #Observable('Bax2_%s' % cpt.name, MatchOnce(Bax(bh3=1) % Bax(bh3=1) ** cpt))
    #}}}
#}}}

#{{{# MODEL MACROS
#{{{# translocate_tBid_Bax_ssa()
def translocate_tBid_Bax_ssa(rate_scaling_factor):
    print("translocate_tBid_Bax_ssa()")

    """
    Note here that for a rate k_det in the deterministic case, translocation is
    k_det * ves_conc * prot_conc.
    Here ves_conc is specified in terms of molar.
    In this case, we have each individual rate as unimolecular with
    k_ssa * prot_conc
    Summed over num_compartments, we have a total rate of 
    k_ssa * prot_conc * num_compartments
    If we've scaled everything up ten-fold (ves_conc -> num_compartments, proteins)
    """

    # Translocation of proteins to vesicles (i.e., from c to m state)
    Parameter('tBid_transloc_kf', (1e-1 * rate_scaling_factor))
    Parameter('tBid_transloc_kr', 0)

    Parameter('Bax_transloc_kf', (1e-2 * rate_scaling_factor))
    #Parameter('Bax_transloc_kr', 1e-3)
    Parameter('Bax_transloc_kr', 1e-2)
...
def build_ssa_model0():
    print "Building SSA model 0:"
    initialize_model()

    translocate_tBid_Bax_ssa(RSF)
    tBid_activates_Bax(1)
    #pores_from_Bax_monomers()
    #Bax_dimerizes()
    #pores_from_Bax_dimers()
#}}}
...
def run_model(tmax=12000):
    xrecs = []
    dr_all = []
    for i in range(0, NUM_SIMS):
        ssa_result = run_ssa(model, t_end=tmax, n_steps=100, cleanup=True)
