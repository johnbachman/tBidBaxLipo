...
if __name__ == '__main__':
    ...
    min_rfu = pymc.Normal('min_rfu', mu=np.min(curve), tau=1 / ((np.min(curve) * 0.05)**2))
    ...
    def exp_fit(max=max_rfu, min=min_rfu, k=k, value=curve):
        ...
        cv = 0.01
        variance = (np.min(curve) * 0.01)**2
        log_lkl = -np.sum(((curve - f)**2) / (2 * variance))
        return log_lkl

    mcmc = pymc.MCMC([max_rfu, min_rfu, k, exp_fit])
    iter=50000
    mcmc.sample(iter=iter, burn=iter/2, thin=10)
    pymc.Matplot.plot(mcmc)

    plt.figure()
    num_to_plot = 500
    max_trace = mcmc.trace('max_rfu')[-num_to_plot:]
    min_trace = mcmc.trace('min_rfu')[-num_to_plot:]
    k_trace = mcmc.trace('k')[-num_to_plot:]
    for i in range(num_to_plot):
        plt.plot(time, exp_func(max_trace[i], min_trace[i], k_trace[i], time), alpha=0.05,
                color='b')
    plt.plot(time, curve, linewidth=2, color='r')
    ...
    # SD**2 = (mean * 0.05)**2
    # tau = 1 / (mean * 0.05)**2
    # Min for time constant = 1 / 120 sec (min timescale)
    # Max for time constant = 1 / 36000 (max timescale)
