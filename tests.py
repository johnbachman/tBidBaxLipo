...
* Membrane perm by Bax depends on cardiolipin (ref)
* tBid-Bax pores can release dextrans up to 2MDa (ref)
* The lipid-induced change is not induced solely by electrostatic interactions. (not shown)
* Bax activation can be inhibited by the membrane-active drug propanolol? (ref)
  ...
  not shown to affect Bax 6A7 (could test propanolol dependence in release assay)
...
* tBid was only able to induce activation of Bax in the presence of liposomes (Fig 6D)
* BclXL + liposomes does not block the 6A7 exposure (Fig 6B, lane 3) One thing
* this doesn't rule out though, is that activated BclXL (pre-equilibrated with
  tBid + liposomes, for example, doesn't prevent 6A7 exposure. Perhaps this is
  dealt with in the Billen paper.
...
* "They conclude that the C-terminal Bax 􏰞-helix is a signal-anchor required
...
* Found that 6A7 exposure didn't require particular lipid composition,
  seemingly contradictory given results showing cardiolipin necessity. "However,
  cardiolipin has only been shown to be required for pore forma- tion, prior
  steps in Bax activation have not been examined previously (19, 20).
